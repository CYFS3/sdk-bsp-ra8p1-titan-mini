...
* 安装成功之后我们双击 eclipse 目录下的 `rasc.exe`，并打开 `Titan_Mini_key_irq` 工程目录下的配置文件 `configuration.xml`：
...
本例程的源码位于 `/project/Titan_Mini_key_irq`。
...
按键中断的源代码位于 `/project/Titan_Mini_key_irq/src/hal_entry.c` 中，当按下 USER_KEY 按键，会改变绿色 LED 的亮灭状态。
